...
# It does not cluster haplotypes that have above m% of missing data
# to run this code include flag -m (max threshold of missing data allowed per haplotype) ex:
# python H12_H2H1_Py3_v2.py VariantSites_ChrX_RA_100.txt 100 -o outH12.txt -w 265 -j 50 -d 0 -m 0.1
...
def clusterSingleWindow(inFile, outFile, windowTot, distanceThreshold, numStrains, singleWindow,missing):
# This definiton calculates H12, H2, and H1 in a single window centered around the coordinate specified by the user. If the coordinate is not in the file or within the bounds of a defineable window, then an error message is outputted.
    ...
    if center!=-1: # this means that the center has not be found
        ...
        runAllDefs(flies, center, distanceThreshold, inFile, outFile, window,missing)

    else:
        print("ERROR: Specified coordinate was not found in file or within a defineable window")

######################
def clusterHaplotypes(inFile, outFile, windowTot, jump, distanceThreshold, numStrains,missing):
...
def downsample(flies,n_downsample):
    ...
    return flies
#######################
def runAllDefs(flies, center, distanceThreshold, inFile, outFile, window,missing):
# This definition runs all other definitions to identify haplotypes, their clusters, and summary statistics. This is run for every analysis window.
        ...
        [haps_clumped, haps_clumped_count] = clusterDiffs(haps, distanceThreshold,missing)
...
def clusterDiffs(haps, distanceThreshold,missing):
        ...
        if (key1 in compared) == False:
            ...
            for key2 in haps.keys():
                if ((haps[key2][0] in haps_clumped[key1]) == False) and ((key2 in compared) == False):
                    [distance, s1]= hamming_distance_clump(key1, key2, distanceThreshold,missing)
...
#############################
def hamming_distance_clump(s1, s2, distanceThreshold,missing_thresh):
    ...
    if numN_s1 >= int(len(s1)*missing_thresh) or numN_s2 >= int(len(s2)*missing_thresh): #if hap has more than missing_thresh missing data
        distance =len(s1)
    else:
        distance = 0
...
def mkOptionParser():
    ...
    parser.add_option("-m", "--missingDataRate",      type="float",       default=0.1,    help="max missing data per haplotype")
...
def main():
    ...
    missing = options.missingDataRate
    ...
    if singleWindow !=-100:
        clusterSingleWindow(inFile, outFile, windowTot, distanceThreshold, numStrains, singleWindow,missing)
    else:
        clusterHaplotypes(inFile, outFile, windowTot, jump, distanceThreshold, numStrains,missing)
